...
def categorize_dish(dish_name, dish_ingredients):
    ...
    for item in categories:
        if ingredients_set.issubset(item[0]):
            return dish_name + ": " + item[1]
...
def tag_special_ingredients(dish):
    ...
    return (
        ...
        set(
            ingredient
            for ingredient in set(dish_ingredients).intersection(SPECIAL_INGREDIENTS)
        ),
    )
...
def singleton_ingredients(dishes, intersection):
    ...
    result = set()
    clean_dishes = [ingredients.difference(intersection) for ingredients in dishes]

    for ingredient_list in clean_dishes:
        for ingredient in ingredient_list:
            dishes_with_ingredient = [
                ingredient for dish in clean_dishes if ingredient in dish
            ]
            if dishes_with_ingredient.count(ingredient) == 1:
