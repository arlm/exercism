...
describe("Largest Series Product", () => {
  ...
  test("can get the largest product of a big number", () => {
    expect(largestProduct("73167176531330624919225119674426574742355349194934", 6)).toEqual(23520);
  });
  ...
  test("rejects span longer than string length", () => {
    expect(() => largestProduct("123", 4)).toThrow(new Error("Span must be smaller than string length"));
  });
  ...
  test("rejects empty string and nonzero span", () => {
    expect(() => largestProduct("", 1)).toThrow(new Error("Span must be smaller than string length"));
  });

  test("rejects invalid character in digits", () => {
    expect(() => largestProduct("1234a5", 2)).toThrow(new Error("Digits input must only contain digits"));
  });

  test("rejects negative span", () => {
    expect(() => largestProduct("12345", -1)).toThrow(new Error("Span must be greater than zero"));
  });
});
