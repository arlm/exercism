...
describe("Grains", () => {
  describe("returns the number of grains on the square", () => {
    ...
    test("square 0 raises an exception", () => {
      expect(() => square(0)).toThrow(new Error("square must be between 1 and 64"));
    });

    test("negative square raises an exception", () => {
      expect(() => square(-1)).toThrow(new Error("square must be between 1 and 64"));
    });

    test("square greater than 64 raises an exception", () => {
      expect(() => square(65)).toThrow(new Error("square must be between 1 and 64"));
    });
  });
