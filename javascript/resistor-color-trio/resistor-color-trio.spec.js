...
describe("Resistor Color Trio", () => {
  test("Orange and orange and black", () => {
    expect(new ResistorColorTrio(["orange", "orange", "black"]).label).toEqual(makeLabel({ value: 33, unit: "ohms" }));
  });

  test("Blue and grey and brown", () => {
    expect(new ResistorColorTrio(["blue", "grey", "brown"]).label).toEqual(makeLabel({ value: 680, unit: "ohms" }));
  });

  test("Red and black and red", () => {
    expect(new ResistorColorTrio(["red", "black", "red"]).label).toEqual(makeLabel({ value: 2, unit: "kiloohms" }));
  });
  ...
  // optional: error
  test("Invalid color", () => {
    expect(() => new ResistorColorTrio(["yellow", "purple", "black"]).label).toThrowError(/invalid color/);
  });
});
