...
describe("React module", () => {
  ...
  test("compute cell takes inputs in correct order", () => {
    ...
    const computeCell = new ComputeCell(inputCells, (inputs) => inputs[0].value + inputs[1].value * 10);
  ...
  test("compute cells can depend on other compute cells", () => {
    ...
    const timesThirty = new ComputeCell([inputCell], (inputs) => inputs[0].value * 30);

    const sum = new ComputeCell([timesTwo, timesThirty], (inputs) => inputs[0].value + inputs[1].value);
  ...
  test("callbacks fire only when output values change", () => {
    const inputCell = new InputCell(1);
    const output = new ComputeCell([inputCell], (inputs) => (inputs[0].value < 3 ? 111 : 222));
  ...
  test("callbacks should only be called once, even if multiple dependencies change", () => {
    ...
    const output = new ComputeCell([plusOne, minusOne2], (inputs) => inputs[0].value * inputs[1].value);
  ...
  test("callbacks should not be called if dependencies change but output value doesn't change", () => {
    ...
    const minusOne = new ComputeCell([inputCell], (inputs) => inputs[0].value - 1);

    const alwaysTwo = new ComputeCell([plusOne, minusOne], (inputs) => inputs[0].value - inputs[1].value);
